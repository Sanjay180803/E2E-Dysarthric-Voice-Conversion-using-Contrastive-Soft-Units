    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            if rank == 0:
                start_b = time.time()
            x, y, _, y_mel = batch
            y = torch.autograd.Variable(y.to(device, non_blocking=False).float())  # Convert to float
            y_mel = torch.autograd.Variable(y_mel.to(device, non_blocking=False).float())  # Convert to float
            y = y.unsqueeze(1)
            x = {k: torch.autograd.Variable(v.to(device, non_blocking=False).float()) for k, v in x.items()}  # Convert to float
                ...
                if steps % a.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):
                            x, y, _, y_mel = batch
                            x = {k: v.to(device, non_blocking=False).float() for k, v in x.items()}  # Convert to float32
                            ...
                            y_mel = torch.autograd.Variable(y_mel.to(device, non_blocking=False).float())
