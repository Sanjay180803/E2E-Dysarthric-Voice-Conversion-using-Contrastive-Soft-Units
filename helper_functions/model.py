...
class Encoder(nn.Module):
    def __init__(self, discrete: bool = False, upsample: bool = True):
        super().__init__()
        if discrete:
            # 0..999 are real token IDs, 1000 is used as padding
            self.embedding = nn.Embedding(1001, 256, padding_idx=1000)
        else:
            self.embedding = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.embedding is not None:
            x = self.embedding(x)  # shape [B, T, 256]
        x = self.prenet(x)        # shape [B, T, 256]
        x = self.convs(x.transpose(1, 2)).transpose(1, 2)  # shape [B, T, 512]
        return x
